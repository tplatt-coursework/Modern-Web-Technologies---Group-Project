...
func tcp_receive(bytes):
	if tcp_client.get_status() == StreamPeerTCP.STATUS_CONNECTED:
		...
		if data[0]!=0: 
			...
			$Response_Code.text = "Response Code: " +str(json["code"])+" "+HTTP_CODE[int(json["code"])]
			print("  Response: "+str(json["response"]))
			$Server_Response.text = "Response: "+str(json["response"])
			print()
...
func _on_timer_timeout() -> void:
	...
	$Status_Label.text = "Connection Status: " + str(STATUS[status])
